...
proof.
  ...
  + wp.
    ...
    ecall{1} (rCONP round{1}). 
    by skip => />; smt(get_setE).
  by auto.
qed.

phoare keys_expand_jazz_correct k : 
  [M.keys_expand : 
    arg = k ==> forall i, 0 <= i < 11 => res.[i] = key_i k i] = 1%r.
proof. conseq keyExpansion_E (aes_keyExpansion_correct k) => // /#. qed.
...
phoare aes_rounds_jazz_correct k m : 
  [ M.aes_rounds : (forall i, 0 <= i < 11 => rkeys.[i] = key_i k i) /\ in_0 = m 
    ==>
    res = aes k m] = 1%r.
proof. by conseq aes_rounds_E (aes_rounds_correct k m) => // /#. qed.

equiv aes_E : M.aes ~ Aes.aes : ={arg} ==> ={res}.
proof. by proc; call aes_rounds_E; call keyExpansion_E; wp; skip. qed.

phoare aes_jazz_correct k n : [M.aes : arg = (k,n) ==> res = aes k n] = 1%r.
proof. conseq aes_E (aes_correct k n) => /#. qed.
...
phoare invaes_jazz_correct k n : [M.invaes : arg = (k,n) ==> res = invaes k n] = 1%r.
proof. conseq invaes_E (invaes_correct k n) => /#. qed.
