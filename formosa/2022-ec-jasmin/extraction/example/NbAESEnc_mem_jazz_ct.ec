...
module M = {
  ...
  proc enc (cptr:W64.t, kptr:W64.t, nptr:W64.t, pptr:W64.t) : unit = {
    ...
    leakages <- LeakAddr([(W64.to_uint (kptr + (W64.of_int 0)))]) :: leakages;
    aux <- (loadW128 Glob.mem (W64.to_uint (kptr + (W64.of_int 0))));
    k <- aux;
    leakages <- LeakAddr([(W64.to_uint (nptr + (W64.of_int 0)))]) :: leakages;
    aux <- (loadW128 Glob.mem (W64.to_uint (nptr + (W64.of_int 0))));
    n <- aux;
    leakages <- LeakAddr([]) :: leakages;
    aux <@ aes (k, n);
    mask <- aux;
    leakages <- LeakAddr([(W64.to_uint (pptr + (W64.of_int 0)))]) :: leakages;
    aux <- (loadW128 Glob.mem (W64.to_uint (pptr + (W64.of_int 0))));
    p <- aux;
    leakages <- LeakAddr([]) :: leakages;
    aux <@ xor (mask, p);
    c <- aux;
    leakages <- LeakAddr([]) :: leakages;
    aux <- c;
    leakages <- LeakAddr([(W64.to_uint (cptr + (W64.of_int 0)))]) :: leakages;
    Glob.mem <-
    storeW128 Glob.mem (W64.to_uint (cptr + (W64.of_int 0))) (aux);
    return ();
  }
  ...
  proc dec (pptr:W64.t, kptr:W64.t, nptr:W64.t, cptr:W64.t) : unit = {
    ...
    mask <- aux;
    leakages <- LeakAddr([(W64.to_uint (cptr + (W64.of_int 0)))]) :: leakages;
    aux <- (loadW128 Glob.mem (W64.to_uint (cptr + (W64.of_int 0))));
    c <- aux;
    leakages <- LeakAddr([]) :: leakages;
    aux <@ xor (mask, c);
    p <- aux;
    ...
    leakages <- LeakAddr([(W64.to_uint (pptr + (W64.of_int 0)))]) :: leakages;
    Glob.mem <-
    storeW128 Glob.mem (W64.to_uint (pptr + (W64.of_int 0))) (aux);
    return ();
  }
}.
